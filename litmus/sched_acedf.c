...
/* We prevent the system from becoming fully utilized
 If either of these values are violated, then the system is repartitioned. 
 For example, if NUMBER_OF_WITHELD_CPUS equals 0.5 in a system with 12 processors. 
 If your estimated total weight is greater than 11.5, then the system will reset. 
 Similarly, if a single processor takes more than MAX_CPUS_UTIL, then the system
 will repartition. */
...
/*	Because we build arrays to hold the information, it's a lot easier
	to define macros for the largest possible values. If you 
	need more clusters or more tasks, then change these values and recompile
*/

#define MAX_CLUSTERS 4
#define MAX_TASKS 100

static int ACEDF_MAX_SERVICE_LEVEL = 3;
static double PERCENT_CHANGE_TRIGGER = 0.125;
...
/* Number of times per second that the system is weighted at a minimum; */
static const lt_t nanosecondsBetweenReweights_acedf = 1000000000/4; 

/* Number of times per second that the system is repatitionined at a maximum */
static const lt_t nanosecondsBetweenRepartitions_acedf = 1000000000*2; 


/* For the first 10 seconds, nothing happens. This allows the system to establish a base weight */
static lt_t initialStableWindowTime_acedf=((lt_t)10)*((lt_t)1000000000); 

/* Time used to keep track of the last time the cluster was reweighted */
static lt_t lastReweightTime_acedf[MAX_CLUSTERS]; 

/* Time the first task starts */
static lt_t initialStartTime_acedf;

/* Time of the last repartitioning of the system */
static lt_t lastRepartitionTime_acedf; 

/* If 1, under utilized. if 0, nothing. -1 if over utilized*/
static int changeNow_acedf; 
...
static noinline void repartition_tasks_acedf(int clusterID){
	...
	/* Note: On my 12 core (24 virtual processors) */
...
static noinline void adjust_all_service_levels_acedf(int triggerReweightNow, int clusterID){
	...
	const double repartition_trigger = 0.9; 
	const int max_level = ACEDF_MAX_SERVICE_LEVEL; 
	double calculationFactor;
	double min_qos;
	double max_qos;
	...
	if( triggerReweightNow!=0 ) { 
		...
		*/
		...
		/* (min_qos > 0) && (max_qos > 0)  && ((max_qos/min_qos) > repartition_trigger
		looks to see if these the ratio of the largest QoS to the smallest QoS is greater than the
		repartition trigger. We need to (min_qos > 0) and (max_qos > 0) to make sure the
		number is positive and not zero
		*/
...
static noinline int job_completion(struct task_struct *t, int forced)
{
	...
	const int max_level = ACEDF_MAX_SERVICE_LEVEL;
	const int lowest_level = 0; 
	const double percentChangeTrigger = PERCENT_CHANGE_TRIGGER; /* If the task's weight changes by this percentage */
	 ...
	 */
	calculate_estimated_execution_cost_acedf(t,alphaValue,betaValue); 
	...
	if( (get_estimated_weight(t) > old_est_weight*(1+percentChangeTrigger)) ||
		(get_estimated_weight(t) < old_est_weight*(1-percentChangeTrigger))) {
		triggerReweightNow = 1;
	}
...
static struct task_struct* acedf_schedule(struct task_struct * prev)
{
	...
	if (schedule_normal_path == -1 ){
		...
		/* We run this path if we migrate a task between processors */
		/* Link pending task if we became unlinked. */
		if (!entry->linked)
			link_task_to_cpu(__take_ready(&cluster->domain), entry);
